...
# revised by Maxime Lopez, 2025-02-05 for bug related to column sorting
#############################################################
#
...
PerSIMPER_overall <- function(matrixSIMP,
  ...
  # The PerSIMPER Overall function requests exactly the same required arguments (matrixSIMP & Groups)
  ...
  warning("This overall approach is under development")
  ...
  dp2 <- permatfull(matrixSIMP, fixedmar = "both", mtype = dataTYPE,
                    times = Nperm)
  ### Permutation with fixed rows sums
  dp3 <- permatfull(matrixSIMP, fixedmar = "rows", mtype = dataTYPE,
                    times = Nperm)
  ...
  if(parallelComputing == TRUE){
    ...
    warning("If permutation end up in infinite loop, log are written in WD")

    #Initiation of CPU nodes
    cl <- makeCluster(detectCores() / 2) # use one less core than the available to not overload the system registerDoParallel(cl
    registerDoParallel(cl)
    ParallelPerSIMPER <- foreach(i = 1:Nperm) %dopar% {
      ...
      ### Computing average contribution to OAD for all groups
      tempMultiSimp2 <- simp2[[1]]$average
      tempMultiSimp3 <- simp3[[1]]$average
      tempMultiSimp4 <- simp4[[1]]$average

      for(m in 2:length(simp2))
      {
        tempMultiSimp2 <- tempMultiSimp2 + simp2[[m]]$average
        tempMultiSimp3 <- tempMultiSimp3 + simp3[[m]]$average
        tempMultiSimp4 <- tempMultiSimp4 + simp4[[m]]$average
      }

      ### Output for parallel computing
      list(simp2 = sort(tempMultiSimp2, decreasing = TRUE),
           simp3 = sort(tempMultiSimp3, decreasing = TRUE),
           simp4 = sort(tempMultiSimp4, decreasing = TRUE))
  ...
  # dn2 <- apply(df2, 2, sort)#These three lines were creating spurious results, deleted in V1.7
  # dn3 <- apply(df3, 2, sort)#These three lines were creating spurious results, deleted in V1.7
  # dn4 <- apply(df4, 2, sort)#These three lines were creating spurious results, deleted in V1.7
  UpO <- apply(df4, 2, quantile, 0.975)
  LoO <- apply(df4, 2, quantile, 0.025)
  MedO <- apply(df4, 2, median)

  UpB <- apply(df2, 2, quantile, 0.975)
  LoB <- apply(df2, 2, quantile, 0.025)
  MedB <- apply(df2, 2, median)

  UpG <- apply(df3, 2, quantile, 0.975)
  LoG <- apply(df3, 2, quantile, 0.025)
  MedG <- apply(df3, 2, median)

  ### Addition of the three null models envelope to the SIMPER empirical plot created Line 60
  if (plotSIMPER == TRUE) {
    lines(UpG, lty="dotted", lwd=2, col="#009900")
    lines(LoG, lty="dotted", lwd=2, col="#009900")
    lines(UpO, lty="dotted", lwd=2, col="#FF9933")
    lines(LoO, lty="dotted", lwd=2, col="#FF9933")
  ...
  ListResults <- list(EcartCarreLog = DataMeanCarreLog, mat = matrixSIMP,
                      ContriPercentage = Pourcent_Contribution, UpOrange = UpO,
                      DownOrange = LoO, MedOrange = MedO,
                      UpBlue = UpB, DownBlue = LoB, MedBlue = MedB,
                      UpGreen = UpG, DownGreen = LoG, MedGreen = MedG,
                      dnOrange = df4, dnBlue = df2, dnGreen = df3)
